...
# 设置日志
logging.basicConfig(filename='inference_resnext34.log', level=logging.INFO, 
...
# ResNeXt3D基本块（与训练代码一致）
class ResNeXtBlock3D(nn.Module):
    expansion = 1
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, cardinality=32, base_width=4):
        ...
        self.conv1 = nn.Conv3d(in_channels, width, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm3d(width)
        
        self.conv2 = nn.Conv3d(width, width, kernel_size=3, stride=stride, padding=1, groups=cardinality, bias=False)
        self.bn2 = nn.BatchNorm3d(width)
        
        self.conv3 = nn.Conv3d(width, out_channels * self.expansion, kernel_size=1, stride=1, padding=0, bias=False)
...
# 3D ResNeXt34模型（与训练代码一致）
class ResNeXt34_3D(nn.Module):
    def __init__(self, block=ResNeXtBlock3D, layers=[3, 4, 6, 3], num_classes=4, cardinality=32, base_width=4):
        super(ResNeXt34_3D, self).__init__()
        self.in_channels = 64
...
# 推断函数（仅替换模型加载和文件名标识）
def inference():
    # 数据路径（修改为ResNeXt对应的模型路径）
    data_dir = Path(r"C:\Users\PC\PycharmProjects\thu_xwh\Val_Data\scratch_imagesTs")
    model_path = r"C:\Users\PC\PycharmProjects\thu_xwh\Codes\Model\unicom20251018\f1_resnext34_fold5\best_model.pth"
    ...
    # 初始化ResNeXt34_3D模型
    model = ResNeXt34_3D(num_classes=4, cardinality=32).to(device)
    try:
        model.load_state_dict(torch.load(model_path, map_location=device))
        logging.info(f"Loaded ResNeXt34 model weights from {model_path}")
    ...
    for case_id in cases:
        ...
        # 保存预测结果（修改文件名标识为resnext34）
        output_path = data_dir / f"{case_id}_f1_resnext34_fold5_gt_5015.npy"
        try:
